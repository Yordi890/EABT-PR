...
describe('SupplyService', () => {
    ...
    describe('addSupply', () => {
        ...
        it('should throw ConflictException if supply already exists', async () => {
            ...
            mockSupplyRepository.insertSupply.mockRejectedValue(new PrismaClientKnownRequestError(
                "Not found", { code: 'P2002', clientVersion: '6.10.0' }
            ));

            await expect(supplyService.addSupply(supplyDto)).rejects.toThrow(ConflictException);
        });
    });
    ...
    describe('modifySupply', () => {
        it('should modify a supply', async () => {
           const name = 'pesticida';
            ...
            expect(mockSupplyRepository.saveSupply).toHaveBeenCalledWith(name, newSupply);
        });
        ...
        it('should throw NotFoundException if supply does not exist', async () => {
            ...
            mockSupplyRepository.saveSupply.mockRejectedValue(new PrismaClientKnownRequestError(
                "Not found", { code: 'P2025', clientVersion: '6.10.0' }
            ));

            await expect(supplyService.modifySupply(name, newSupply)).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('deleteSupply', () => {
        ...
        it('should throw NotFoundException if supply does not exist', async () => {
            ...
            mockSupplyRepository.removeSupply.mockRejectedValue(new PrismaClientKnownRequestError(
                "Not found", { code: 'P2025', clientVersion: '6.10.0' }
            ));

            await expect(supplyService.deleteSupply(name)).rejects.toThrow(NotFoundException);
        });
    });
});
