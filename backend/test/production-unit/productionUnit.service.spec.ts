...
describe('ProductionUnitService', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                {provide: ProductionUnitRepository, useValue: mockProductionUnitRepository},
            ],
        }).compile();

        productionUnitService = module.get<ProductionUnitService>(ProductionUnitService);
    ...
    describe('listAllProductionUnits', () => {
        it('should return an array of production units', async () => {
            ...
            mockProductionUnitRepository.retrieveAllProductionUnit.mockResolvedValue(result);

            expect(await productionUnitService.listAllProductionUnit()).toEqual(result);
            expect(mockProductionUnitRepository.retrieveAllProductionUnit).toHaveBeenCalled();
        });
    });
    ...
    describe('addProductionUnit', () => {
        it('should add a production unit', async () => {
            const productionUnitDto: ProductionUnitDto = {
                name: 'Ceferino Lopez', address: 'Calle 5ta #8'
            };

            await productionUnitService.addProductionUnit(productionUnitDto);
            expect(mockProductionUnitRepository.insertProductionUnit).toHaveBeenCalledWith(productionUnitDto);
        });
        ...
        it('should throw ConflictException if production unit already exists', async () => {
            ...
            mockProductionUnitRepository.insertProductionUnit.mockRejectedValue(new PrismaClientKnownRequestError(
                "Not found", {code: 'P2002', clientVersion: '6.10.0'}
            ));

            await expect(productionUnitService.addProductionUnit(productionUnitDto)).rejects.toThrow(ConflictException);
        });
    });
    ...
    describe('modifyProductionUnit', () => {
        it('should modify a production unit', async () => {
            ...
            expect(mockProductionUnitRepository.saveProductionUnit).toHaveBeenCalledWith(name, newProductionUnit);
        });
        ...
        it('should throw NotFoundException if production unit does not exist', async () => {
            ...
            mockProductionUnitRepository.saveProductionUnit.mockRejectedValue(new PrismaClientKnownRequestError(
                "Not found", {code: 'P2025', clientVersion: '6.10.0'}
            ));

            await expect(productionUnitService.modifyProductionUnit(name, newProductionUnit)).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('deleteProductionUnit', () => {
        it('should delete a production unit', async () => {
            ...
            mockProductionUnitRepository.removeProductionUnit.mockResolvedValue(undefined);

            await productionUnitService.deleteProductionUnit(name);
            expect(mockProductionUnitRepository.removeProductionUnit).toHaveBeenCalledWith(name);
        });

        it('should throw NotFoundException if production unit does not exist', async () => {
            const name = 'Augusto Lopez';

            mockProductionUnitRepository.removeProductionUnit.mockRejectedValue(new PrismaClientKnownRequestError(
                "Not found", {code: 'P2025', clientVersion: '6.10.0'}));

            await expect(productionUnitService.deleteProductionUnit(name)).rejects.toThrow(NotFoundException);
        });
    });
})